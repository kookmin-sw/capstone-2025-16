<!-- 
  문자열 연산자 컴포넌트
  특징:
  - 문자열 연산자 지원 (eq, neq, startsWith, endsWith, contains)
  - 단일 값 또는 여러 값 지원
-->
<script lang="ts">
  ...
  const availableOperators = [
    ...
    { type: 'startsWith', label: '시작하는 글자' },
    { type: 'endsWith', label: '끝나는 글자' },
    { type: 'contains', label: '포함' }
  ];
  ...
  function getActiveOperators(): string[] {
    ...
    return active.length > 0 ? active : ['contains']; // 기본값
  }
