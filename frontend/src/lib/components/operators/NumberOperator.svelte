<!-- 
  Number operator component
  Features:
  - Supports number comparison operators (eq, neq, gt, lt, gte, lte)
  - Supports range specification (gte + lte)
  - Supports single value or multiple values
-->
<script lang="ts">
  import { createEventDispatcher } from 'svelte';
  
  // Number operator type definition
  ...
  const availableOperators = [
    ...
    { type: 'gt', label: 'Greater than (>)', icon: '>' },
    { type: 'lt', label: 'Less than (<)', icon: '<' },
    { type: 'gte', label: 'Greater than or equal to (≥)', icon: '≥' },
    { type: 'lte', label: 'Less than or equal to (≤)', icon: '≤' }
  ];
        ...
        {#if isMultipleMode[type]}
          ...
          <div class="space-y-3">
            <div class="flex items-center space-x-2">
              <input 
                type="number" 
                class="w-32 rounded-md border border-gray-300 bg-white py-1.5 px-3 text-sm shadow-sm transition-all focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
