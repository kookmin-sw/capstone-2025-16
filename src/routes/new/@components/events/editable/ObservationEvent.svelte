<script>
	import DateOperator from '../operators/DateOperator.svelte';
	import NumberOperator from '../operators/NumberOperator.svelte';
	import { createEventDispatcher } from 'svelte';

	const dispatch = createEventDispatcher();

	// 필터 상태 변수들
	let first = $state(false);
	let age = $state({});
	let gender = $state({});
	let date = $state({});
	let observationType = $state({});
	let visitType = $state({});
	let valueAsNumber = $state({});
	let valueAsString = $state({ value: '' });
	let valueAsConcept = $state({});
	let qualifierType = $state({});
	let unitType = $state({});
	let source = $state({});
	let providerSpecialty = $state({});

	// 툴팁 표시 상태
	let showTooltip = $state({});

	// 툴팁 표시/숨김 함수
	function toggleTooltip(key) {
		showTooltip[key] = !showTooltip[key];
	}

	$effect(() => {
		dispatch('add', {
			type: 'observation',
			filters: {
				first,
				age,
				gender,
				date,
				observationType,
				visitType,
				valueAsNumber,
				valueAsString,
				valueAsConcept,
				qualifierType,
				unitType,
				source,
				providerSpecialty
			}
		});
	});
</script>

<div class="flex flex-col w-full gap-y-8">
	<!-- First Observation -->
	<div class="flex flex-col gap-y-2 w-full">
		<span class="text-left">First observation record of the patient</span>
		{#if first}
			<button on:click={() => (first = false)} class="rounded-full bg-blue-200 px-2 py-1 text-xs text-blue-500 w-fit"> Enabled </button>
		{:else}
			<button on:click={() => (first = true)} class="rounded-full bg-pink-200 px-2 py-1 text-xs text-pink-500 w-fit"> Disabled </button>
		{/if}
	</div>

	<!-- Age at Observation -->
	<div class="flex flex-col gap-y-2 w-full">
		<p>Age at observation:</p>
		<NumberOperator id="age-at-observation" on:value={({ detail }) => (age = detail)} />
	</div>

	<!-- Gender -->
	<div class="flex flex-col gap-y-2 w-full">
		<p>Gender:</p>
		<select class="w-full p-2 border rounded" bind:value={gender}>
			<option value="">Select</option>
			<option value="M">Male</option>
			<option value="F">Female</option>
		</select>
	</div>

	<!-- Date -->
	<div class="flex flex-col gap-y-2 w-full">
		<p>Observation date:</p>
		<DateOperator id="observation-date" on:value={({ detail }) => (date = detail)} />
	</div>

	<!-- Observation Type -->
	<div class="flex flex-col gap-y-2 w-full">
		<p>Observation Type:</p>
		<NumberOperator id="observation-type" on:value={({ detail }) => (observationType = detail)} />
	</div>

	<!-- Visit Type -->
	<div class="flex flex-col gap-y-2 w-full">
		<p>Visit Type:</p>
		<NumberOperator id="visit-type" on:value={({ detail }) => (visitType = detail)} />
	</div>

	<!-- Value As Number -->
	<div class="flex flex-col gap-y-2 w-full">
		<p>Value As Number:</p>
		<NumberOperator id="value-as-number" on:value={({ detail }) => (valueAsNumber = detail)} />
	</div>

	<!-- Value As String -->
	<div class="flex flex-col gap-y-2 w-full">
		<p>Value As String:</p>
		<input 
			type="text" 
			class="w-full p-2 border rounded"
			bind:value={valueAsString.value} 
			placeholder="Enter Text Value"
		/>
	</div>

	<!-- Value As Concept -->
	<div class="flex flex-col gap-y-2 w-full">
		<p>Value As Concept:</p>
		<NumberOperator id="value-as-concept" on:value={({ detail }) => (valueAsConcept = detail)} />
	</div>

	<!-- Qualifier Type -->
	<div class="flex flex-col gap-y-2 w-full">
		<p>Qualifier Type:</p>
		<NumberOperator id="qualifier-type" on:value={({ detail }) => (qualifierType = detail)} />
	</div>

	<!-- Unit Type -->
	<div class="flex flex-col gap-y-2 w-full">
		<p>Unit Type:</p>
		<NumberOperator id="unit-type" on:value={({ detail }) => (unitType = detail)} />
	</div>

	<!-- Source -->
	<div class="flex flex-col gap-y-2 w-full">
		<p>Source:</p>
		<NumberOperator id="source" on:value={({ detail }) => (source = detail)} />
	</div>

	<!-- Provider Specialty -->
	<div class="flex flex-col gap-y-2 w-full">
		<p>Provider Specialty:</p>
		<NumberOperator id="provider-specialty" on:value={({ detail }) => (providerSpecialty = detail)} />
	</div>
</div> 